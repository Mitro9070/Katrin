...
function NewsBlockSlide({ name, data, className = '' }) {
    ...
    if (loading) return <div className={"block-slide " + className + ' block-slide-loader'} style={{ width: '1095px', height: '237px' }}><Loader /><p className="name-block-list">{name}</p></div>;
    if (error) return <div className={"block-slide " + className + ' block-slide-loader'} style={{ width: '1095px', height: '237px' }}><p>{error}</p><p className="name-block-list">{name}</p></div>;
